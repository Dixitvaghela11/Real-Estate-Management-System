...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_user'])) {
    ...
    if (!empty($_FILES['uimage']['name'])) {
        $uimage = file_get_contents($_FILES['uimage']['tmp_name']);
        $update_image_query = "UPDATE users SET uimage = ? WHERE uid = ?";
        $update_image_stmt = $conn->prepare($update_image_query);
        $update_image_stmt->bind_param("si", $uimage, $user_id);
        $update_image_stmt->execute();
    }

    // Update user details
    $update_user_query = "UPDATE users SET uname = ?, uemail = ?, uphone = ? WHERE uid = ?";
    
    $update_user_stmt = $conn->prepare($update_user_query);
    $update_user_stmt->bind_param("sssi", $uname, $uemail, $uphone, $user_id);
...
<head>
    ...
    <title>GARO ESTATE | Edit User</title>
        ...
        <div class="container">
            <div class="row">
                <div class="page-head-content">
                    <h1 class="page-title animate__animated animate__fadeInDown"><a style="color: white;" href="index.php">Home</a> | Edit User</h1>               
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="container">
            <div class="col-md-8 col-md-offset-2">
                <div class="box-for overflow">
                    <div class="col-md-12 col-xs-12 edit-user-blocks">
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="uname" name="uname" value="<?php echo htmlspecialchars($user['uname']); ?>" required>
    </div>
    <div class="form-group">
        <label for="uemail">User Email :</label>
        <input type="email" class="form-control" id="uemail" name="uemail" value="<?php echo htmlspecialchars($user['uemail']); ?>" required>
    </div>
    <div class="form-group">
        <label for="uphone">User Phone :</label>
        <input type="text" class="form-control" id="uphone" name="uphone" value="<?php echo htmlspecialchars($user['uphone']); ?>">
    </div>
    <div class="form-group">
    ...
    <div class="text-center">
        <button type="submit" class="btn btn-default" name="edit_user">Update User</button>
    </div>
</form>
    ...
    <script>
        ...
        // Example usage
        // showAlert('success', 'Success', 'User details updated successfully.', 'edit_user_profile.php');
    </script>
