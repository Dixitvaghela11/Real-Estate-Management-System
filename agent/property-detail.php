...
// Retrieve agent details from the agents table using aid from the property table
$aid = $property['aid'];
if ($aid !== null) {
...
} else if ($property['admin_id'] == 2) {
    // Property was created by admin
    $agent = array(
        'aid' => null,
        'aname' => 'Admin',
...
if (!$agent) {
    echo "<script>showAlert('error', 'Error', 'Agent/Admin information not found.');</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['form_type']) && $_POST['form_type'] == 'inquiry_form') {
        ...
        if ($stmt->execute()) {
            echo "<script>showAlert('success', 'Success', 'your inquiry submitted successfully!', 'property-detail.php?p_id=$p_id');</script>";
        } else {
        ...
        $stmt->close();
        exit();
    } elseif (isset($_POST['form_type']) && $_POST['form_type'] == 'review_form') {
        // Handle Property Review Form
        ...
        if (!$p_id || (!$user_id && !$agent_id) || !$review_title || !$review_content || !$rating) {
            echo "<script>showAlert('error', 'Submission Failed', 'All fields are required!');</script>";
        } else {
            $sql = "INSERT INTO reviews (uid, aid, p_id, review_title, review_content, rating) 
                    VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("iiissd", $user_id, $agent_id, $p_id, $review_title, $review_content, $rating);

            if ($stmt->execute()) {
                echo "<script>showAlert('success', 'Success', 'review added successfully!', 'property-detail.php?p_id=$p_id');</script>";
            } else {
                echo "<script>showAlert('error', 'Submission Failed', 'Failed to add review. Please try again.');</script>";
            }
...
<div style="background-color: #96a0b5;" class="dealer-widget">
    <div class="dealer-content">
        <div class="inner-wrapper">
                ...
                <div class="clear">
                    <div class="col-xs-4 col-sm-4 dealer-face">
                        <a href="useragent.php?aid=<?php echo $agent['aid']; ?>">
                            <img src="data:image/jpeg;base64,<?php echo base64_encode($agent['aimage']); ?>" class="img-circle" alt="Agent Image">
                        </a>
                    </div>
                    <div class="col-xs-8 col-sm-8">
                        <h3 class="dealer-name">
                            <a href="useragent.php?aid=<?php echo $agent['aid']; ?>"><?php echo htmlspecialchars($agent['aname']); ?></a>
                            <span>Real Estate Agent</span>
                        </h3>
                        <div class="dealer-social-media">
                            <!-- Only show social links if they exist -->
                            <a class="twitter" target="_blank" href="<?php echo isset($agent['twitter']) ? htmlspecialchars($agent['twitter']) : '#'; ?>">
                                <i class="fa fa-twitter"></i>
                            </a>
                            <a class="facebook" target="_blank" href="<?php echo isset($agent['facebook']) ? htmlspecialchars($agent['facebook']) : '#'; ?>">
                                <i class="fa fa-facebook"></i>
                            </a>
                            <a class="gplus" target="_blank" href="<?php echo isset($agent['gplus']) ? htmlspecialchars($agent['gplus']) : '#'; ?>">
                                <i class="fa fa-google-plus"></i>
                            </a>
                            <a class="linkedin" target="_blank" href="<?php echo isset($agent['linkedin']) ? htmlspecialchars($agent['linkedin']) : '#'; ?>">
                                <i class="fa fa-linkedin"></i>
                            </a>
                            <a class="instagram" target="_blank" href="<?php echo isset($agent['instagram']) ? htmlspecialchars($agent['instagram']) : '#'; ?>">
            ...
            <?php else: ?>
                ...
                <div class="text-center">
                    ...
                    <ul class="dealer-contacts">
                        <li><i class="pe-7s-user strong"></i> Administrator</li>
            ...
            <?php if ($aid !== null): ?>
                <div class="clear">
                    <ul class="dealer-contacts">
                        <li><i class="pe-7s-map-marker strong"></i> <?php echo isset($property['office_address']) ? htmlspecialchars($property['office_address']) : "N/A"; ?></li>
                        <li><i class="pe-7s-mail strong"></i> <?php echo htmlspecialchars($agent['aemail']); ?></li>
                        <li><i class="pe-7s-call strong"></i> <?php echo htmlspecialchars($agent['aphone']); ?></li>
                    </ul>
                    <p><?php echo isset($agent['content']) ? htmlspecialchars($agent['content']) : "No additional details available."; ?></p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <!-- <div class="panel panel-default sidebar-menu similar-property-wdg wow fadeInRight animated">
        <div class="panel-heading">
            <h3 class="panel-title">Property Inquiry</h3>
        </div>
        <div class="panel-body recent-property-widget">
            <form action="" method="POST">
                <input type="hidden" name="form_type" value="inquiry_form"> 
                <input type="hidden" name="p_id" value="<?php echo $property_id; ?>" /> 
