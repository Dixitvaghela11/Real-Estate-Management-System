    ...
    <style>
        ...
        .custom-alert.processing .custom-alert-buttons {
            display: none;
        }

        .custom-alert.processing .custom-alert-message {
            margin-bottom: 0;
        }
    </style>
    ...
    <script>
        function showAlert(type, title, message, redirectUrl = null) {
            ...
            // Only add click-outside handling for non-processing alerts
            if (type !== 'processing') {
            ...
            alert.onkeydown = function(event) {
                if (event.key === 'Escape' && type !== 'processing') {
                    alert.classList.remove('show');
                }
            };

            // Set focus to the OK button when the alert is shown (except for processing)
            if (type !== 'processing') {
